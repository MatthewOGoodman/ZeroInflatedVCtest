zeroinflVC =
  # This is the model-fitting function modified from the 'pscl' package
  # so that it also outputs the score (gradfn), in addition to the density (loglikfun) and residuals (residfn)
  # that were previously ouput. We make use of the score.resids in the VC test
function (formula, data, subset, na.action, weights, offset,
  ...
  residPoisson <- function(parms) {
    ...
    wres_zero <- ifelse(Y1, -1/(1 - muz) * linkobj$mu.eta(etaz),
                        (linkobj$mu.eta(etaz) - exp(clogdens0) * linkobj$mu.eta(etaz))/dens0)
    resid = cbind(wres_count * weights, wres_zero * weights)
    colnames( resid ) = c("count", "zero")
    return( resid )
  }
  residGeom <- function(parms) {
    eta <- as.vector(X %*% parms[1:kx] + offsetx)
    mu <- exp(eta)
    etaz <- as.vector(Z %*% parms[(kx + 1):(kx + kz)] + offsetz)
    muz <- linkinv(etaz)
  ...
  residNegBin <- function(parms) {
    ...
    wres_zero <- ifelse(Y1, -1/(1 - muz) * linkobj$mu.eta(etaz),
                        (linkobj$mu.eta(etaz) - exp(clogdens0) * linkobj$mu.eta(etaz))/dens0)
    wres_theta <- theta * ifelse(Y1, digamma(Y + theta) -
                                   digamma(theta) + log(theta) - log(mu + theta) + 1 - (Y + theta)/(mu + theta),
                                 exp(-log(dens0) + log(1 - muz) + clogdens0) *
                                   (log(theta) - log(mu + theta) + 1 - theta/(mu + theta)))
    resid =  cbind(wres_count * weights, wres_zero * weights) # Question: How to handle wres_theta?
    colnames( resid ) = c("count", "zero")
    return( resid )
  }
  ...
  gradfun <- switch(dist, poisson = gradPoisson, geometric = gradGeom,
                    negbin = gradNegBin)
  residfun <- switch(dist, poisson = residPoisson, geometric = residGeom,
                    negbin = residNegBin)
  ...
  rval <- list(coefficients = list(count = coefc, zero = coefz),
               residuals = res,
               score.resids = residfun(c(coefc,coefz)),
               fitted.values = Yhat, optim = fit, method = method,

               control = ocontrol, start = start,
               ...
               n = nobs, df.null = nobs - 2, df.residual = nobs - (kx + kz + (dist == "negbin")),
